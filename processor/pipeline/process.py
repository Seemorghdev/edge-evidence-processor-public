...
Reuses the shared finalization, registration, and staging primitives verbatim:
  - the five-second POSIX advisory spool lock (processor.pipeline.lock);
  - the shared raw-media evidence validator (processor.pipeline.verification);
  - staging publish/fsync/quarantine helpers (processor.pipeline.staging);
  - deterministic manifest assembly + serialization (processor.pipeline.finalize);
  - the pure processor core (processor.processing.fingerprint).
...
from processor.pipeline import finalize, migration_history, staging, verification
from processor.pipeline.finalize import FinalizeError
...
def process_artifact(request: ProcessRequest) -> tuple[str, "DerivedPlan"]:
    ...
    from processor.pipeline import lock as spool_lock_mod
